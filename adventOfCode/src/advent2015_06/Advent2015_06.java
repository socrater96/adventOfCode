package advent2015_06;



public class Advent2015_06 {
	static void parte1(String input) {
		int[][] matriz = new int[1000][1000];	
		String[] inputArray=input.split("\n");
		for(String linea: inputArray) {
			switch(linea.substring(0,7)) {
				case "turn of":
					for(int i=Integer.parseInt(linea.split(" ")[2].split(",")[0]);i<=Integer.parseInt(linea.split(" ")[4].split(",")[0]);i++){
						for(int j=Integer.parseInt(linea.split(" ")[2].split(",")[1]);j<=Integer.parseInt(linea.split(" ")[4].split(",")[1]);j++) {
							matriz[i][j]=0;
						}
					}
					break;
				case "turn on":
					...
					break;
				case "toggle ":
					for(int i=Integer.parseInt(linea.split(" ")[1].split(",")[0]);i<=Integer.parseInt(linea.split(" ")[3].split(",")[0]);i++){
						for(int j=Integer.parseInt(linea.split(" ")[1].split(",")[1]);j<=Integer.parseInt(linea.split(" ")[3].split(",")[1]);j++) {
							if(matriz[i][j]==1)
								matriz[i][j]=0;
							else
								matriz[i][j]=1;
